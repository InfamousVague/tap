...
// Wire-format types matching the relay's API. Unchanged from
// TapMac/Models/Models.swift — Codable shapes are stable across the
// Watch / iOS / Mac apps, so keep the field names identical to avoid
// fork drift between platforms.
...
struct Server: Codable, Identifiable, Hashable {
    ...
    static func == (lhs: Server, rhs: Server) -> Bool { lhs.id == rhs.id }

    var commandCount: Int { commands?.count ?? 0 }
    var displayStatus: String { status ?? "unknown" }

    /// Mapped from the relay's free-form `status` string for the
    /// widget. "ok"/"online"/"active" all read as online; anything
    /// else (or nil) reads as offline. Keeps the widget surface tied
    /// to one bool rather than re-parsing strings in two places.
    var widgetOnline: Bool {
        switch (status ?? "").lowercased() {
        case "ok", "online", "active", "up": return true
...
struct ConfigResponse: Codable { let servers: [Server] }
...
struct KeysResponse: Codable { let keys: [SSHKey] }
